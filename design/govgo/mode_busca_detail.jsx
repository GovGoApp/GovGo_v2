...
function buildSimilarityAccent(score) {
  const t = Math.max(0, Math.min(1, Number(score || 0)));
  if (t >= 0.8) return "var(--sim-1)";
  if (t >= 0.6) return "var(--sim-2)";
  if (t >= 0.4) return "var(--sim-3)";
  if (t >= 0.2) return "var(--sim-4)";
  return "var(--sim-5)";
}
                ...
                <div style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
                  <div style={{
                    ...
                    <div style={{
                      position: "absolute",
                      inset: 0,
                      width: `${Math.max(0, Math.min(1, Number(k.barScore || 0))) * 100}%`,
                      background: k.toneColor || "var(--sim-3)",
                      borderRadius: 999,
                    }}/>
                  </div>
                  <div className={k.mono ? "mono" : ""} style={{
                    fontSize: 14.75, fontWeight: 600,
                    color: k.tone === "risk" ? "var(--risk)" : k.toneColor || "var(--ink-1)",
