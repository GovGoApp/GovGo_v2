...
def create_connection() -> Optional[Any]:
    """Cria conexão psycopg2 com base V1."""
    try:
        if psycopg2 is None:
            dbg('SQL', 'psycopg2 indisponivel; conexao BD desabilitada no laboratorio local')
            return None
        _load_env_priority()
...
def create_engine_connection():
    ...
    try:
        if create_engine is None:
            dbg('SQL', 'SQLAlchemy indisponivel; engine BD desabilitada no laboratorio local')
            return None
        _load_env_priority()
        host = os.getenv('SUPABASE_HOST', 'aws-0-sa-east-1.pooler.supabase.com')
        user = os.getenv('SUPABASE_USER')
